...
- Group real scan candidates by tool. Each row shows its actual size, safety, useful detail, and full path for discovered locations. Shut down devices on older iOS runtimes, old XCTest clones, and project Derived Data require individual review. Current or active simulators, recent test devices, archives, device support, Android SDKs and AI models remain visible but cannot be selected.
...
- Keep monitored-but-managed storage separate from both. It includes more than logs: current or active simulator data, recent test devices, device support, archives, Android SDKs, emulators, and AI models. It is never selectable or passed to cleanup; explain this from the summary.
- Allow shut down simulators from older installed iOS runtimes as unchecked review candidates. Revalidate their runtime and state at cleanup, and remove only the selected device via `simctl delete`. Keep the runtime installed in Xcode.
